...
type CaseStudy = {
  ...
  era?: 'Classic' | 'Evergreen' | 'Trending'; context?: string; evidence?: string; decision?: string;
...
const eras = ['All eras','Classic','Evergreen','Trending'] as const;
...
export default function Home() {
  ...
  const [era, setEra] = useState<EraFilter>('All eras');
  ...
  const filtered = useMemo(() => cases.filter((item) => {
    ...
    const eraMatch = era === 'All eras' || getEra(item) === era;
    ...
    return topicMatch && eraMatch && searchMatch;
  }), [topic, era, search]);
  ...
  return (
    <main id="top">
      ...
      <section className="casebook" id="casebook">
        ...
        <div className="eraFilters" aria-label="Filter by case era">
          <span>Browse by era</span>
          {eras.map((item) => <button key={item} className={era === item ? 'active' : ''} onClick={() => setEra(item)} aria-pressed={era === item}>{item === 'Trending' ? 'Trending · 2024–2026' : item}</button>)}
        </div>
...
function getEra(item: CaseStudy): Exclude<EraFilter, 'All eras'> {
